    ...
    func updateModelsFromDataBase(data: [String : AnyObject]) {
        ...
        for firebaseResume in firebaseConverter.convertDataToResumeModels(data: data) {
            if let firebaseResume = firebaseResume  {
                ...
                copyAttributes(ofModels: educationModels, toResume: coreDataResume)
            }
        }
...
extension ResumeModelHandler {
    ...
    func modelValues(ofModel: UIDable?, filter elements: [String]) -> [(title: String, description: String)] {
        guard let ofModel = ofModel as? NSManagedObject else { return []}
        var result = [(String, String)]()
        for propertyKey in ofModel.entity.propertiesByName.keys.filter({ !elements.contains($0)}) {
            let description = propertyKey.titleCase
            if ofModel.value(forKey: propertyKey) is String? {
                if let propertyValue = ofModel.value(forKey: propertyKey) {
                    result.append((propertyValue as! String, description))
                } else {
                    result.append(("", description))
                }
